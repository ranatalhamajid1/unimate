...
export function AiStudyPlanModal({ isOpen, onClose, onSuccess }: Props) {
  ...
  const [isUpgradeRequired, setIsUpgradeRequired] = useState<boolean>(false);
  ...
  async function handleGenerate(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        if (data.code === "UPGRADE_REQUIRED" || res.status === 403) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/40 backdrop-blur-xs animate-fade-in">
      ...
      >
        ...
        {isUpgradeRequired ? (
          <div className="mt-5 space-y-4 rounded-2xl border border-purple-500/20 bg-gradient-to-b from-purple-500/10 via-[var(--color-surface)] to-[var(--color-surface)] p-6 text-center animate-fade-in">
            <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-purple-500/15 text-purple-600 dark:text-purple-400">
              <Sparkles className="h-6 w-6" />
            </div>

            <div className="space-y-1.5">
              <div className="inline-flex items-center gap-1.5 rounded-full border border-purple-500/30 bg-purple-500/10 px-2.5 py-0.5 text-[10.5px] font-semibold uppercase tracking-wider text-purple-600 dark:text-purple-400">
                UniMate Pro Feature
              </div>
              <h3 className="text-base font-bold text-[var(--color-text)]">
                Unlock AI Study Plans
              </h3>
              <p className="text-xs text-[var(--color-text-2)] leading-relaxed max-w-sm mx-auto">
                AI Study Plans analyze your enrolled courses, imminent exam countdowns, and class timetable gaps to automatically generate realistic, conflict-free revision schedules.
              </p>
            </div>
            ...
            <div className="pt-2 flex flex-col sm:flex-row items-center justify-center gap-2.5">
              <Link
                ...
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-xl bg-purple-600 px-5 py-2.5 text-xs font-semibold text-white shadow-sm hover:bg-purple-500 transition-all cursor-pointer"
              >
              ...
              <button
                ...
                className="w-full sm:w-auto rounded-xl border border-[var(--color-border)] px-4 py-2.5 text-xs font-medium text-[var(--color-text-2)] hover:bg-[var(--color-surface-2)] transition-colors cursor-pointer"
              >
                Dismiss
              </button>
            </div>
