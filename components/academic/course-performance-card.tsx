      ...
      <div>
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <h3 className="mt-2 text-[16px] font-semibold text-[var(--color-text)] leading-snug line-clamp-1">
              {courseName}
            </h3>
          </div>
        </div>
        ...
        <div className="mt-4 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-2)] p-3.5">
          <div className="flex items-center justify-between">
            <span className="flex items-center gap-1.5 text-[12px] font-medium text-[var(--color-text-2)]">
              <Award className="h-3.5 w-3.5 text-[var(--color-text-3)]" />
              Course Grade
            </span>
            <button
              onClick={() => onEditGrade(courseId)}
              className="inline-flex items-center gap-1 text-[12px] font-semibold text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 transition"
            >
              {grade ? (
                <>
          ...
          {grade ? (
            ...
            <p className="mt-2 text-[13px] font-normal text-[var(--color-text-3)] italic">
              Grade not added
            </p>
          )}
        </div>
        ...
        {/* ── Attendance Section ── */}
        <div className="mt-3 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-2)] p-3.5">
          <div className="flex items-center justify-between">
            <span className="flex items-center gap-1.5 text-[12px] font-medium text-[var(--color-text-2)]">
              <CalendarCheck className="h-3.5 w-3.5 text-[var(--color-text-3)]" />
              Attendance
            </span>
            <button
              onClick={() => onEditAttendance(courseId)}
              className="inline-flex items-center gap-1 text-[12px] font-semibold text-emerald-600 dark:text-emerald-400 hover:text-emerald-700 dark:hover:text-emerald-300 transition"
            >
              {totalClasses > 0 ? (
                <>
                  <Edit2 className="h-3 w-3" />
